...
\begin{definition}[Inverse Problem with Gaussian Noise]\label{defn:inverse-problem}
...
\begin{definition}[Linear Inverse Problem]\label{defn:linear-inverse-problem}
...
\begin{proposition}[Optimization via Sampling
    \parencite{hwangLaplaceMethodRevisited1980, kongDiffusionModelsConstrained2024}
] \label{prop:opt-sampling}
...
\begin{definition}[Stein Score] \label{defn:stein-score}
...
\begin{definition}[Backwards Denoising Process \parencite{douDiffusionPosteriorSampling2023}] \label{defn:backwards-process}
...
\begin{definition}[Sequential Monte Carlo\footnote{Also known as Particle Filtering.} for Filtering] \label{defn:smc}
